...
export default function Receipt() {
  ...
  return (<div className="bg-gray-50 flex flex-col">
    <div className="flex">
    ...
    <div className="w-full pt-60 px-40 flex-grow mono">
      <div className="w-1/3 relative mb-40 mx-auto flex flex-col justify-center items-center">
        <Image src={LogoSymbol} alt="Logo" className="w-2/3 mb-6"/>
        <Image src={Logotype} alt="Logo" className="w-full"/>
      </div>
      <div className="mb-40">
        <p className="ft-6 !leading-[1.5] text-center">Bar code services S.A. de C.V.<br/>
      ...
      <div className="mb-40 grid grid-cols-2">
        <p className="ft-6 !leading-[1.5] text-left">Fecha emisión:</p>
        <p className="ft-6 !leading-[1.5] text-right">{receiptInfo.issueDate}</p>
        <p className="ft-6 !leading-[1.5] text-left">Cliente:</p>
        <p className="ft-6 !leading-[1.5] text-right">{receiptInfo.client}</p>
        <p className="ft-6 !leading-[1.5] text-left">Referencia:</p>
        <p
          className="ft-6 !leading-[1.5] text-right uppercase">{`PAY-${receiptInfo.refCode}`}</p>
      </div>
      <div className="grid py-12 grid-cols-2 border-dashed border-b-2 border-black">
        <p className="ft-6 !leading-[1.5] font-bold text-left uppercase italic">Concepto</p>
        <p className="ft-6 !leading-[1.5] font-bold text-right uppercase italic">Monto</p>
      </div>
      <div className="py-12 grid grid-cols-2 border-dashed border-b-2 border-black">
        <p className="ft-6 !leading-[1.5] text-left">{receiptInfo.concept}</p>
        <p className="ft-6 !leading-[1.5] text-right">{receiptInfo.amount()}</p>
      </div>
      <div className="mb-20 py-12 grid grid-cols-2">
        <p className="ft-6 !leading-[1.5] text-left">IVA:</p>
        <p className="ft-6 !leading-[1.5] text-right">{receiptInfo.vat().value}</p>
        <p className="ft-6 !leading-[1.5] text-left">A pagar:</p>
        <p className="ft-6 !leading-[1.5] text-right">{receiptInfo.vat().totalDue}</p>
      </div>
      <div className="mb-20 py-12 grid grid-cols-2">
        <p className="ft-6 !leading-[1.5] text-left">Fecha límite pago:</p>
        <p className="ft-6 !leading-[1.5] text-right">{receiptInfo.dueDate}</p>
      </div>
      <div>
        <p className="ft-6 !leading-[1.5] text-center">Gracias</p>
      </div>
    </div>
    <p className='p-20'>{receiptInfo.id}</p>
  </div>);
}